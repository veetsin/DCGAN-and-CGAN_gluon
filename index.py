    ...
    def save_image(self, rsp_data, word):
        ...
        for image_info in rsp_data['imgs']:
            ...
            try:
                ...
                print("coder+1,已有" + str(self.__counter) + "张coder")
                self.__counter += 1
        return
...
keyword = ['韩国　美女','日本　美女','美女']
for key_word in keyword:
    if __name__ == '__main__':
        crawler = Crawler(0.05)  # 抓取延迟为 0.05
    
        # crawler.start('美女', 10, 2)  # 抓取关键词为 “美女”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
        crawler.start(key_word,20,1)  # 抓取关键词为 “二次元 美女”，总数为 10 页（即总共 10*60=600 张），起始抓取的页码为 1
        # crawler.start('帅哥', 5)  # 抓取关键词为 “帅哥”，总数为 5 页（即总共 5*60=300 张）

japan_stars = ['户田惠梨香','新垣结衣','吉高由里子','石原里美','深田恭子','菅野美穗','苍井优','有村架纯','田中丽奈','渡边麻友','板野友美','道重沙由美','真野恵里菜','吉濑美智子','泽尻绘里香','天海佑希','仓科加奈','坂井泉水','常盘贵子','竹内结子','本田翼']
for key_word in japan_stars:
    if __name__ == '__main__':
        crawler = Crawler(0.05)  # 抓取延迟为 0.05
    
        # crawler.start('美女', 10, 2)  # 抓取关键词为 “美女”，总数为 1 页（即总共 1*60=60 张），开始页码为 2
        crawler.start(key_word,3,1)  # 抓取关键词为 “二次元 美女”，总数为 10 页（即总共 10*60=600 张），起始抓取的页码为 1
        # crawler.start('帅哥', 5)  # 抓取关键词为 “帅哥”，总数为 5 页（即总共 5*60=300 张）
